...
countrycode <- function(
  ...
  if (
    !is.null(nomatch) &&
      (length(nomatch) != 1) &&
      (length(nomatch) != length(sourcevar))
  ) {
  ...
  if (
    ...
    stop(sprintf(
      "The `origin` argument must be a string of length 1 equal to one of these values: %s.",
  ...
  if (
    ...
    stop(
      "The `destination` argument must be a string or a vector of strings where each element is equal to one of the column names in the conversion directory (by default: `codelist`)."
    )
  }

  if (!inherits(dictionary, "data.frame")) {
    stop(
      "The `dictionary` argument must be a data frame or tibble with codes as columns."
    )
  }

  dups = anyDuplicated(stats::na.omit(dictionary[, origin])) > 0
  if (dups) {
  ...
  if (is.null(custom_dict)) {
    # only for built-in dictionary
    # unicode.symbol breaks uppercase on Windows R-devel 2022-02-02; rejected by CRAN
    if (
      inherits(origin_vector, 'character') &&
        !grepl('country|unicode.symbol', origin)
    ) {
...
countrycode_convert <- function(
  ...
  if (origin_regex) {
    ...
    matches[lengths(matches) > 1] <- `class<-`(
      NA,
      class(dict[[destination]])
    )
    ...
    if (is.character(origin_vector) && is.numeric(dictionary[[origin]])) {
      msg <- sprintf(
        "To convert a `%s` code, `sourcevar` must be numeric.",
        origin
      )
      stop(msg, call. = FALSE)
    }
  ...
  if (is.null(nomatch)) {
    if (sane_sourcevar) {
      ...
      class(sourcevar)[1] == "factor" &&
        class(destination_vector)[1] == "character"
    ) {
      ...
      if (length(destination_list) > 0) {
        if (!is.null(custom_match)) {
          destination_list <- destination_list[
            !names(destination_list) %in% names(custom_match)
          ]
        }
        if (length(destination_list) > 0) {
          destination_list <- lapply(destination_list, function(k) {
            paste(k, collapse = ',')
          })
