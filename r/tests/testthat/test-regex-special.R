...
iso3c_of <- function(name) {
  countrycode(name, 'country.name', 'iso3c', warn = TRUE)
}
no_warn_iso3c_of <- function(name) {
  countrycode(name, 'country.name', 'iso3c', warn = FALSE)
}
cowc_of <- function(name) countrycode(name, 'country.name', 'cowc', warn = TRUE)
no_warn_cowc_of <- function(name) {
  countrycode(name, 'country.name', 'cowc', warn = FALSE)
}
...
test_that('the Netherlands Antilles are all matched correctly', {
  ...
  expect_equal(
    no_warn_iso3c_of('sint maarten, saba, and sint eustatius'),
    NA_character_
...
test_that('Micronesia is not Federated States of Micronesia', {
  ...
  expect_equal(
    countrycode('Mikronesien', 'country.name.de', 'iso3c', warn = FALSE),
    NA_character_
  )
  expect_equal(
    countrycode('Micron\u00e9sie', 'country.name.fr', 'iso3c', warn = FALSE),
    NA_character_
  )
  expect_equal(
    countrycode('Micronesia', 'country.name.it', 'iso3c', warn = FALSE),
    NA_character_
  )
  expect_equal(
    countrycode('Micronesia', 'country.name.es', 'iso3c', warn = FALSE),
    NA_character_
  )
  ...
  expect_equal(
    countrycode('F.S. Mikronesien', 'country.name.de', 'iso3c'),
    'FSM'
  )
  ...
  expect_equal(
    countrycode('FS Micron\u00e9sie', 'country.name.fr', 'iso3c'),
    'FSM'
  )
  # Italian: qualified forms → FSM, FS abbreviation → FSM
  expect_equal(
    countrycode('Stati Federati di Micronesia', 'country.name.it', 'iso3c'),
    'FSM'
  )
  expect_equal(
    countrycode('Micronesia (Stati Federati di)', 'country.name.it', 'iso3c'),
    'FSM'
  )
  expect_equal(countrycode('FS Micronesia', 'country.name.it', 'iso3c'), 'FSM')
  # Spanish: qualified forms → FSM, FS abbreviation → FSM
  expect_equal(
    countrycode('Estados Federados de Micronesia', 'country.name.es', 'iso3c'),
    'FSM'
  )
  expect_equal(
    countrycode(
