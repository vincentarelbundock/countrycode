...
test_that('nomatch argument works correctly when sourcevar is a factor', {
  ...
  expect_equal(
    countrycode(factor('XXX'), 'iso3c', 'iso2c', nomatch = c('YYY')),
    'YYY'
  )
  expect_equal(
    countrycode(
      factor(c('XXX', 'USA', 'YYY')),
      'iso3c',
      'iso2c',
      nomatch = c('YY')
    ),
  ...
  expect_equal(
    countrycode(factor('XXX'), 'iso3c', 'iso2c', nomatch = NULL),
    'XXX'
  )
  expect_equal(
    countrycode(
  ...
  expect_equal(
    countrycode(factor('XXX'), 'iso3c', 'iso2c', nomatch = NA, warn = FALSE),
    NA_character_
  )
  expect_equal(
    countrycode(
