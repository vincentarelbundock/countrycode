...
test_that('custom dictionary without regex', {
  test_func <- function(iso3c_codes) {
    countrycode(
      iso3c_codes,
      'iso3c',
      'country.name.en',
  ...
  expect_equal(
    test_func(c('USA', 'CAN', 'XXX')),
    c('United States', 'Canada', NA)
  )
})

test_that('custom dictionary with regex', {
  test_func <- function(countrynames) {
    countrycode(
      countrynames,
  ...
  expect_equal(
    test_func(c('U.S.A.', 'West Germany', 'XXX')),
    c('USA', 'DEU', NA)
  )
})
...
test_that('canadian provinces test', {
  dict <- data.frame(
    'regex' = c('qu.bec', 'alberta', 'british.columbia'),
  ...
  expect_equal(
    countrycode('QC', 'abbreviation', 'province', custom_dict = dict),
    'Quebec'
  )
})
