...
un <- c('en', 'fr', 'es', 'ru', 'zh') %>%
  ...
  mutate(
    ...
    # Cabo Verde
    un.name.fr = ifelse(
      un.name.fr == 'Cabo Verde (République de)',
      'Cabo Verde',
      un.name.fr
    ),
    un.name.ar = ifelse(un.name.ar == 'الرأس الأخضر', 'كابو فيردي', un.name.ar),
    # Congo
    un.name.zh = ifelse(un.name.zh == '刚果（布）', '刚果', un.name.zh),
    # Costa Rica
    un.name.ar = ifelse(un.name.en == 'Costa Rica', 'كوستاريكا', un.name.ar),
    # Côte d’Ivoire
    un.name.en = ifelse(
      un.name.en == 'Côte D\'Ivoire',
      'Côte d’Ivoire',
      un.name.en
    ),
    un.name.es = ifelse(
      un.name.es == 'Côte D\'Ivoire',
      'Côte d’Ivoire',
      un.name.es
    ),
    un.name.ru = ifelse(
      un.name.ru == 'Кот-д\'Ивуар',
      'Кот-д’Ивуар',
      un.name.ru
    ),
    # Czechia
    un.name.en = ifelse(un.name.en == 'Czech Republic', 'Czechia', un.name.en),
    un.name.fr = ifelse(
      un.name.fr == 'République tchèque',
      'Tchéquie',
      un.name.fr
    ),
    un.name.es = ifelse(un.name.es == 'República Checa', 'Chequia', un.name.es),
    un.name.ru = ifelse(
      un.name.ru == 'Чешская Республика',
      'Чеxия',
      un.name.ru
    ),
    un.name.ar = ifelse(
      un.name.ar == 'الجمهورية التشيكية',
      'تشيكيا', # I think
      un.name.ar
    ),
    # DPRK
    un.name.en = ifelse(
      un.name.en == 'Democratic People\'s Republic of Korea',
      'Democratic People’s Republic of Korea',
      un.name.en
    ),
    # Gambia
    un.name.en = ifelse(
      un.name.en == 'Gambia (Republic of The)',
      'Gambia',
      un.name.en
    ),
    un.name.fr = ifelse(
      un.name.fr == 'Gambie (République de)',
      'Gambie',
      un.name.fr
    ),
    un.name.es = ifelse(
      un.name.es == 'Gambia (República de)',
      'Gambia',
      un.name.es
    ),
    # Guinea-Bissau
    un.name.en = ifelse(
      un.name.en == 'Guinea Bissau',
      'Guinea-Bissau',
      un.name.en
    ),
    un.name.es = ifelse(
      un.name.es == 'Guinea Bissau',
      'Guinea-Bissau',
      un.name.es
    ),
    # Iran
    un.name.fr = ifelse(
      un.name.fr == 'Iran (République islamique d\')',
    ...
    # Niger
    un.name.es = ifelse(un.name.es == 'Niger', 'Níger', un.name.es),
    # North Macedonia
    un.name.ar = ifelse(
      un.name.ar == 'جمهورية مقدونيا الشمالية',
      'مقدونيا الشمالية',
      un.name.ar
    ),
    # South Sudan
    un.name.ar = ifelse(
      un.name.ar == 'جمهورية جنوب السودان',
      'جنوب السودان',
      un.name.ar
    ),
    # Timor-Leste
    un.name.ar = ifelse(
      un.name.ar == 'تيمور- ليشتي',
      'تيمور-ليشتي',
      un.name.ar
    ),
    ...
    # United Kingdom
    un.name.fr = ifelse(
      un.name.fr == 'Royaume-Uni de Grande-Bretagne et d\'Irlande du Nord',
      'Royaume-Uni de Grande-Bretagne et d’Irlande du Nord',
      un.name.fr
    ),
    # United States
    un.name.fr = ifelse(
      un.name.fr == 'États-Unis d\'Amérique',
      'États-Unis d’Amérique',
      un.name.fr
    ),
    # Venezuela
    un.name.en = ifelse(
      un.name.en == 'Venezuela, Bolivarian Republic of',
      'Venezuela (Bolivarian Republic of)',
      un.name.en
    ),
    un.name.zh = ifelse(
      un.name.zh == '委内瑞拉玻利瓦尔共和国',
      '委内瑞拉（玻利瓦尔共和国）',
      un.name.zh
    ),
    # Add country column
