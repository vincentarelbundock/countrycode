...
if (!l10n_info()$`UTF-8`) {
  warning('Running in a non-UTF-8 locale!')
}
...
filelist <- utils::unzip(zipfile, list = TRUE)
...
get_names <- function(filename) {
  ...
  lang <- tibble(code = names(lang), name = unlist(lang)) %>%
    ...
    mutate(
      variant = if_else(is.na(variant), 'cldr.name', paste0('cldr.', variant))
    ) %>%
    spread(variant, name) %>%
    mutate_at(
      vars(-iso2c, -cldr.name),
      .funs = list(~ if_else(is.na(.), cldr.name, .))
    )
...
cldr <- cldr %>%
  filter(
    !nchar(iso2c) > 2, # remove rows with 3 digit codes
    !cldr.name.en %in% bad, # remove 'bad' countries
    !cldr.name.af %in% c('XA', 'XB')
  ) %>% # weird no matching regex
  select(-iso2c) %>%
  setNames(tolower(names(.)))

# duplicate column names
if (anyDuplicated(colnames(cldr)) > 0) {
