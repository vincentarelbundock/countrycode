...
dat <- countrycode::codelist %>%
  ...
  mutate(
    countryname = countrycode(countryname, 'country.name', 'country.name.en')
  ) %>%
  filter(
    nchar(alternative) > 4,
...
dat <- bind_rows(a, b) %>%
  ...
  distinct %>%
  rename(country.name.en = countryname, country.name.alt = alternative)

# save to file
countryname_dict <- data.frame(dat)
save(
  countryname_dict,
  file = 'r/data/countryname_dict.rda',
  compress = 'xz',
  version = 2
)
