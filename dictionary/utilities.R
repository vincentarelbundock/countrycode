...
#############################
custom_dict <- read.csv(
  'dictionary/data_regex.csv',
  na = '',
  stringsAsFactors = FALSE
) %>%
  dplyr::select(country.name.en.regex, country.name.en.regex)

CountryToRegex <- function(x, warn = TRUE) {
  countrycode(
    x,
...
SanityCheck <- function(dataset) {
  ...
  if ('year' %in% colnames(dataset)) {
    # panel
    checkmate::assert_false(anyNA(dataset[['year']]))
  }
  checkmate::assert_false(anyNA(dataset[['country.name.en.regex']]))
  ...
  if ('year' %in% colnames(dataset)) {
    ...
    checkmate::assert_true(
      anyDuplicated(dataset[['country.name.en.regex']]) == 0
    )
  }
}
