...
tmpxls <- tempfile(fileext = '.xls')
download.file(url, tmpxls, quiet = TRUE, mode = 'wb')
p5 <- read_excel(tmpxls) %>%
  select(p5n = ccode, p5c = scode, country, year) %>%
  mutate(p5n = as.integer(p5n), year = as.integer(year)) %>%
  # overlapping years; pick one arbitrarily
  filter(
    (year != 1832) | (p5c != 'GCL'), # Gran Colombia vs. Colombia
