...
letter.pair.to.integer <- function(pair) {
  (match(stringr::str_sub(pair, 1, 1), LETTERS) - 1) *
    26 +
    match(stringr::str_sub(pair, 2, 2), LETTERS) -
    1
}

integer.to.letter.pair <- function(int) {
  paste0(LETTERS[int %/% 26 + 1], LETTERS[int %% 26 + 1])
}
...
string.range.expand <- function(string.range) {
  stringr::str_replace_all(
    ...
    stringr::str_replace_all('([:upper:][:upper:])', function(x) {
      letter.pair.to.integer(x) %>% as.character()
    }) %>%
    stringr::str_split('[:space:]') %>%
    unlist() %>%
    purrr::map(~ parse(text = .x) %>% eval()) %>%
    unlist() %>%
    stringr::str_replace_all('([:digit:]+)', function(x) {
      as.integer(x) %>% integer.to.letter.pair()
    })
}
...
unicode.symbol <- iso %>%
  dplyr::mutate(
    unicode.symbol = dplyr::if_else(
      iso2c %in% valid.regions,
      iso2c,
      NA_character_
    )
  ) %>%
...
assertthat::assert_that(all(
  valid.regions %in% c(unicode.symbol$unicode.symbol, exceptional.reservations)
))
...
unicode.symbol %>%
  rowwise %>%
  mutate(
    unicode.symbol = utf8ToInt(unicode.symbol) %>%
