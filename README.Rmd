...
The `custom_dict` argument accepts data frame which can be used as custom dictionaries to create "crosswalks" between arbitrary entities (non-countries). The `countrycode` repository on Github hosts several custom dictionaries. You can obtain the current list by calling:
