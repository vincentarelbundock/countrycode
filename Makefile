.PHONY: help website reference articles dictionary \
	r-document r-check r-install r-test \
	py-install py-test py-lint py-readme py-coverage py-build py-publish
...
dictionary: ## dictionary
	...
	cd python && uv run --all-extras python csv2pickle.py
...
py-readme: ## render the Python README with Quarto
	cd python && uv run quarto render README.qmd
...
py-build: ## build the Python package
	cd python && uv build
