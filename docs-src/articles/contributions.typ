...
#set document(title: [Contributions])

#metadata((
  title: "Contributions",
  summary: "How to add a new country code, contribute a custom dictionary, and set the dictionary attributes countrycode reads.",
)) <website-metadata>
...
- Run `scripts/build-dictionary.R`
...
The `countrycode` repository holds several custom dictionaries: #link("https://github.com/vincentarelbundock/countrycode/tree/main/custom-dictionaries").

To add your own custom dictionary, please make sure that:

+ You save a comma-separated CSV file that looks something like `custom-dictionaries/data_us_states.csv`
...
+ #box[\<NA\>]/blank fields are blank, not the string 'NA' (not RFC 4180, but important here because of Namibia)
...
state_dict <- "https://raw.githubusercontent.com/vincentarelbundock/countrycode/main/custom-dictionaries/data_us_states.csv"
